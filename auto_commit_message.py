...
def get_commit_message(files_tuple):
    ...
    for files, action in [
        ...
        if files:
            ...
            message = (
                message + f" & {action} {files_joined}"
                if message
                else f"{action} {files_joined}"
            )
...
def main():
    try:
        ...
        for change in changes:
            ...
            staging, working, space, file = change[0], change[1], change[2], change[3:]
        ...
        message = get_commit_message(
            files_tuple=(update_files, create_files, delete_files, rename_files)
        )
        if message:
            print(message)
